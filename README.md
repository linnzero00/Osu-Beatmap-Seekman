...
Osu! Beatmap Seekman 是一个基于 Tauri 2 + Rust 的 osu! beatmap 批量搜索与下载工具。它支持按星数、日期、长度、模式、OD/HP/CS/AR/BPM、mania 键数等条件构建候选列表，并通过多个镜像源批量下载到 osu!stable 的 `Songs` 目录，也支持 `.osu` 单文件模式、osu!lazer 曲库去重、AlphaOsu PP 推荐、图包 CSV 导入导出和收藏夹写入。
...
- 修复多个图包任务并行完成时收藏夹目标串入、收藏夹缺失或被后续写入覆盖的问题。
- 每个下载任务在入队时固定保存目标收藏夹和 osu!stable 路径，切换界面选择不再影响已有任务。
- 所有 `collection.db` 写入改为串行处理，并在同组目标不一致时停止提交，避免进一步破坏收藏夹。
- 新增“修复下载收藏夹串入”，可根据已完成任务记录重新分配谱面、补建收藏夹并自动备份数据库。
- 修复拖入图包 CSV 后点击弹窗外侧会误关闭导入确认界面的问题。

## 2.1.8 更新

- 下载任务支持“开始并移至顶部”，所选任务优先运行，其余任务继续排队。
- 修复已缓存项目进度条不显示，并将失败后跳过的项目纳入已处理进度。
- 镜像全部失败后停止自动循环切源，提示谱面可能不可下载并保留失败报告。
- 图包 CSV 导入新增统计确认弹窗，可扫描本地后选择已有收藏夹或新建收藏夹，并一次完成迁移与缺失项下载。
- 图包候选区新增清空列表。

## 2.1.7 更新

- “歌单”功能更名为“图包导入”，兼容 Seekman CSV 与 osu!mania Ladder 图包 CSV，并支持将 CSV 直接拖入窗口。
- 镜像源设置移动到下载页，可单独启用或停用各镜像，并继续调整优先级与混杂模式。
- 下载任务区新增实时下载速度、完成图速率和预计剩余时间，正在下载项目置顶、已缓存项目置底。
- “一键重试”仅重试正在下载和失败的项目，不再重置已缓存、已完成或等待中的项目。
- 图包导入可将歌曲写入已有或新建的 osu!stable 收藏夹，并对缺失标识的 CSV 行给出明确提示。
...
- 全新分页式界面：`设置`、`搜图`、`下载`、`图包导入` 分页拆开，页面更清爽。
...
当前 `.osz` 下载会按用户设置优先使用以下镜像。每个镜像都可单独启用或停用，混杂模式只会在已启用的镜像之间调度：
