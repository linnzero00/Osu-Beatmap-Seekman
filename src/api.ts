...
const browserFallback: Api = {
  ...
  repairDownloadCollections: async () => ({ backupPath: "", managedHashes: 0, missingBeatmaps: 0, collections: [] }),
  exportCollectionPlaylist: async () => "",
  exportBeatmapsetPlaylist: async () => "",
  importSeekmanPlaylist: async () => ({ items: [], exportedAt: "", sourceCollection: "", title: "", author: "", description: "" }),
  applyLocalPlaylistItemsToCollection: async (_stableOsuDir, _collectionName, items) => ({ appliedCount: 0, appliedBeatmapsetCount: 0, missingCount: items.length, missingItems: items }),
  searchBeatmapsets: async () => [],
  searchAlphaRecommendations: async () => [],
  searchUserBestScores: async () => [],
  enqueueDownloads: async () => [],
  startDownloads: async () => ({ ok: false }),
  startDownloadGroup: async () => [],
...
export const api: Api = electronApi ?? (isTauri ? {
  ...
  repairDownloadCollections: () => invoke("repair_download_collections"),
  exportCollectionPlaylist: (stableOsuDir, collectionName, selectedBeatmapIds, playlistTitle, playlistAuthor, playlistDescription) => invoke("export_collection_playlist", { stableOsuDir, collectionName, selectedBeatmapIds, playlistTitle, playlistAuthor, playlistDescription }),
  exportBeatmapsetPlaylist: (items, sourceCollection, playlistTitle, playlistAuthor, playlistDescription) => invoke("export_beatmapset_playlist", { items, sourceCollection, playlistTitle, playlistAuthor, playlistDescription }),
  importSeekmanPlaylist: (path, contents) => invoke("import_seekman_playlist", { path, contents }),
  applyLocalPlaylistItemsToCollection: (stableOsuDir, collectionName, items, commit) => invoke("apply_local_playlist_items_to_collection", { stableOsuDir, collectionName, items, commit }),
  ...
  searchUserBestScores: (request) => invoke("search_user_best_scores", { request }),
  enqueueDownloads: (items) => invoke("enqueue_downloads", { items }),
  startDownloads: () => invoke("start_downloads"),
  startDownloadGroup: (groupId) => invoke("start_download_group", { groupId }),
